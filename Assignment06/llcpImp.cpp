...
// Function to merge 2 sorted lists into a third list
void Merge2AscListsRecur(Node*& xHeadPtr, Node*& yHeadPtr, Node*& zHeadPtr)
{
    //edge case. if both lists are empty then 3rd list will stay empty
    if(xHeadPtr == nullptr && yHeadPtr == 0) return;

    // check to see if x is not empty, if so then check if x data is less than y data if so
    // place x in z if not, place y in z.
    if(xHeadPtr != 0 && (yHeadPtr == 0 || xHeadPtr->data <= yHeadPtr->data))
    {
        zHeadPtr = xHeadPtr;
        xHeadPtr = xHeadPtr->link;
        zHeadPtr->link = 0;
    }
    else
    {
        zHeadPtr = yHeadPtr;
        yHeadPtr = yHeadPtr->link;
        zHeadPtr->link = 0;
    }

    //make recursive call
    Merge2AscListsRecur(xHeadPtr, yHeadPtr, zHeadPtr->link);

} // end of Merge2AscListsRecur()
